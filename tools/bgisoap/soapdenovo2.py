...
def main():
    ...
    parser.add_option("-f", "--output_gap_related_reads", dest="output_gap_related_reads",
        help="Output gap related reads in map step for using SRkgf to fill gap")
    ...
    cmd = "/usr/local/bgisoap/soapdenovo2/bin/SOAPdenovo-63mer all -s %s -K 63 -R -o %s" % (tmp_file,
                                                                                          tmp_dir + "/result")
    print cmd
